...
from src.utils import normalize_fn, append_bias_term, accuracy_fn, macrof1_fn, get_time_analyse
import sys
...
def main(args):
    ...
    fraction_train = 0.8
    rinds = np.random.permutation(xtrain.shape[0])
    n_train = int(fraction_train * xtrain.shape[0])

    train_data = xtrain[rinds[:n_train]]
    train_labels = ytrain[rinds[:n_train]]
    validation_data = xtrain[rinds[n_train:]]
    validation_labels = ytrain[rinds[n_train:]]

    # Calculate the mean and standard deviation of each feature
    mean = np.mean(train_data, axis=0)
    std = np.std(train_data, axis=0)

    train_data_normalized = normalize_fn(train_data,mean,std)
    train_data_normalized_with_bias = append_bias_term(train_data_normalized)

    validation_data_normalized = normalize_fn(validation_data,mean,std)
    validation_data_normalized_with_bias = append_bias_term(validation_data_normalized)

    # Make a validation set (it can overwrite xtest, ytest)
    if not args.test:
        if args.method == "kmeans" or "logistic_regression":
            xtest = validation_data_normalized
        else:
            xtest = validation_data_normalized_with_bias
    ...
    if args.method == "k-means":
        method_obj.graph(60, xtrain, ytrain, xtest, ytest, preds)

    ## Time analysis:
    # Training
    time = get_time_analyse(
        method_obj.fit, xtrain, ytrain
    )
    print(f"Time analysis for training: time = {time}")

    # Prediciting
    time = get_time_analyse(
        method_obj.predict, xtest
    )
    print(f"Time analysis for prediction: time = {time}")
